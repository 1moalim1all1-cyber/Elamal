...
### ⚙️ إعداد base المهم (مصدر شاشة بيضاء شائع)

الموقع دلوقتي شغال تحت مسار فرعي `https://username.github.io/Elamal/` (بدون دومين مخصص لسه)، فلازم دايمًا:
- `base` في `vite.config.ts` = `"/Elamal/"` (لازم يطابق اسم الريبو بالظبط، بحروفه الكبيرة والصغيرة)
- `segmentCount` في `public/404.html` = `1`

لو غيّرت اسم الريبو، أو لو ربطت دومين مخصص بعدين، لازم تغيّر الاتنين مع بعض وترفع (`npm run deploy`) تاني:

| الحالة | base | segmentCount |
|---|---|---|
| رابط GitHub الافتراضي تحت اسم ريبو (الوضع الحالي) | `"/اسم-الريبو/"` | `1` |
| دومين مخصص من الجذر | `"/"` | `0` |

لو نسيت تظبطهم مع بعض، الموقع بيطلع صفحة بيضاء فاضية (لأن ملفات JS/CSS بتتدور عليها في مكان غلط).
