...
function render(d){
  ...
  const about=document.getElementById('aboutImage')||document.querySelector('.about-img>img');if(about&&d.about?.image)about.src=d.about.image;
  const hero=document.getElementById('homeHero');if(hero&&d.hero?.backgroundImage){hero.style.backgroundImage=`linear-gradient(rgba(6,20,38,.78),rgba(7,26,47,.82)),url('${d.hero.backgroundImage}')`;hero.style.backgroundSize='cover';hero.style.backgroundPosition='center'}
  const imageMap={categoryImage1:d.homeImages?.category1,categoryImage2:d.homeImages?.category2,categoryImage3:d.homeImages?.category3,catalogImage:d.homeImages?.catalog};Object.entries(imageMap).forEach(([id,src])=>{const el=document.getElementById(id);if(el&&src)el.src=src});
